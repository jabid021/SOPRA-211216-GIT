		...
		<table class="table">
			<c:forEach items="${employes}" var="e">
				<tr>
					...
					<td>${e.salaire}</td>
					<td>${e.commission}</td>
					<td><a href="${ctx}/employe/edit?id=${e.id}" class="btn btn-primary">editer</a></td>
					<td><a href="${ctx}/employe/delete?id=${e.id}" class="btn btn-danger">supprimer</a></td>
				</tr>
			</c:forEach>
		</table>
		<a href="${ctx}/employe/add" class="btn btn-link">nouvel employe</a>
	</div>
</body>
</html>